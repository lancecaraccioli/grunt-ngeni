...
var validatePackage = function(pkg) {
  if (!pkg.repository) {
    throw new Error("package.json must contain a 'repository' property");
  } else if (!pkg.branchVersion) {
    throw new Error("package.json must contain a 'branchVersion' property");
  }
  return true;
};
/**
...
var getPackage = function() {
  ...
  try {
    var pkg = JSON.parse(fs.readFileSync(path.join(packageFolder, 'package.json'), 'UTF-8'));
  } catch (ex) {
...
var getGitRepoInfo = function() {
  ...
  if (currentPackage.repository && currentPackage.repository.url) {
    //naive regex generalization, todo use git cli or some other structured data
    var GITURL_REGEX = /^(http|https|git|ssh|git\+ssh):\/\/[^\/]*\/([^\/]+)\/(.+).git$/;
 ...
 */
function getBuild() {
  var cmd = shell.exec('git rev-parse --short HEAD', {silent: true});
  if (cmd.output) {
    var hash = cmd.output.replace('\n', '');
  } else {
    throw new Error("Could not extract build hash");
  }
  return 'sha.'+hash;
}
